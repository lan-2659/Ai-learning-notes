...
# 如果没有 就创建 一个模型 第一次训练
# print()
mode_path=os.path.join(os.path.dirname(__file__),'src',"model","mbgd_regressor_model.pkl")
transfer_path=os.path.join(os.path.dirname(__file__),'src',"model","mbgd_transfer.pkl")
# print(mode_path)
...
def train():
    ...
    for epoch in range(epochs):
        ...
        for batch in range(math.ceil(len(x_train)/batch_size)):
            ...
            print(f"第{batch}批次的均方误差为：{e}")
        # end_time=time.time()
        score=model.score(x_test,y_test)
        print(f"训练轮次:{epoch}/{epochs}  score:{score} time:{time.time()-start_time}")
        # 保存模型
