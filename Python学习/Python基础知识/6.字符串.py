...
# 字符串的切片
    ...
    # 注意事项：
    字符串切片会返回一个新字符串，不会改变原字符串
    字符串切片时start和end可以越界(两个同时越界都可以)
    字符串切片时会先根据步长的正负判断区间[start:end)是否可以进行正向或反向遍历，如果不行则会直接返回一个空字符串
...
# 字符串的索引
    ...
    # 注意事项：
    索引时index不可以超范围
    索引返回的是一个字符串（python中不区分单字符和字符串，都统一叫字符串）

    # 举例：
    str = 'hello world'
    print(str[0])                  # 通过索引输出字符串第一个字符
    print(str[-1])                 # 通过索引输出字符串最后一个字符
"""
...
chr(65)            # 返回一个字符串，将Unicode码转换成对应字符
ord("A")           # 返回一个整数，将字符转换成Unicode码

eval("x + y", globals={"x":1}, locals={"y":1})   # 将传入的字符串当做python表达式来执行，并返回执行结果；后两个参数是可选的，用于指定全局变量和局部变量
    # 后两个参数不能用关键字传参，这里只是方便理解
    # eval() 函数的参数查找顺序：
        如果 globals、locals 两个参数没有传参，会直接在调用这个函数的作用域中找 x 和 y
        会先在 locals 中找 x 和 y，如果找不到再到 globals 中找，如果还找不到最后在内置作用域（__builtins__）中找
    # __builtins__ 的作用：包含了 Python 的所有内置函数和内置变量，如import、print()、input()、list、int、True、None等等
    # 可以在 globals字典 中将传入 "__builtins__":None 来禁止使用内置函数和内置变量
"""
