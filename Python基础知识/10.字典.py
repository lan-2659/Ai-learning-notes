...
""" 字典是一种映射类型，字典用"{ }"标识，它是一个无序的“键(key) : 值(value)”对集合。“键(key)”必须使用不可变类型
...
x = {}                     #空字典
x = dict()                     #空字典
dict3 = dict(one=1, two=2, three=3)       #使用了dict方法创建了一个字典,使用此方法时'='前的部分不能加任何引号
        # dict()函数可以传入可迭代对象，但可迭代对象的每一个元素中必须包含两个数据，前者为键， 后者为值
