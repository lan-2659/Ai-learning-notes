...
"""
# 函数的形参定义自左至右的顺序为：
    def 函数名(强制位置参数（'/'前面的参数），/，位置参数，默认参数，解包元组参数，命名关键字参数（*或者*args后面的参数），解包字典参数)
# 命名关键字形参只能通过关键字传参，命名关键字形参也可以设置默认值（但是没有顺序限制）
# 位置形参只能通过位置传参（这个语法需要 python3.8+）
# 普通形参既可以通过位置传参，也可以通过关键字传参

def 函数名(参数1, 参数2, ……, 参数n='默认值'):     # 可以把函数设置成不需要参数, 也可以设置多个参数
    函数体                                       # 设置默认值时等号两边不要留空白
