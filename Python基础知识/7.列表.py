...
list_x[1:3] = [2, 3, 4]           # 切片赋值必须赋可迭代对象, 如列表、元组、字符串等
print(list_x)                           
...
# 列表对象的常用API
"""
list_x.append(3)       # 将传入的数据整体作为一个元素添加到列表末尾, 不返回任何值
list_x.extend([3, 4])  # 将可迭代对象的元素依次逐个添加到列表末尾, 不返回任何值
...
list_x.index(3)     # 查找元素3第一次出现时的位置，并返回该位置，若是元素不存在会报错
