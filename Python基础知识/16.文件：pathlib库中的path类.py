...
path = Path("D:\Python：study\基础语法学习\pi_digits.txt")   # 创建Path类的实例，这个实例创建后会固定，之后再调用时不会发生变化
...
path.write_text('I love programming!')      # 向这个路径写入数据，如果这个路径不存在，会先创建再写入，而且写入的内容会覆盖原有内容
