...
def get_taptap_reviews(name, number):
        # 目标网页URL
    url = f"https://www.taptap.cn/app/{number}/review?mapping=%E5%B7%AE%E8%AF%84&label=0"
    ...
    # 保存为 CSV 文件
    df.to_csv(f'tap游戏_{name}_坏评论数_{len(reviews)}.csv', index=False)  
